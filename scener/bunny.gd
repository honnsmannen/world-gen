...
func _physics_process(_delta : float) -> void:
	...
	if fleeing:
		nav_agent.set_target_location(pos_to_go_to)
...
func _on_Area2D_body_entered(body):
	if body.is_in_group("player"):
		var spelarens_position = body.global_position
		
		if global_position.x > 0 and global_position.y > 0:
			pos_to_go_to = Vector2(global_position.x + 64, global_position.y + 64)
			fleeing = true
